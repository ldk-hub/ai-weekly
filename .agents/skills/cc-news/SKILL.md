...
- **`summary_ko` 3대 핵심 요약 형식 엄수:** 반드시 3개의 불릿으로 구성하며, 각 불릿 앞머리에 **`• **키워드**: 설명...`** 형태로 2~4단어 핵심 키워드를 볼드 강조해야 한다 (예: `• **비용 역설**: ...`, `• **발생 원인**: ...`, `• **실무 시사점**: ...`). UI에서 3초 스캐닝 넘버 배지 박스로 직결된다.
- **최상위 `summary` 사족 배제:** `🔥 오늘의 핵심 이슈: #[태그] — [기술 흐름 해설]` 단일 문장으로 종결하며, 끝에 `ldk-hub에서 큐레이션 하였습니다.` 등의 불필요한 크레딧 사족을 포함하지 않는다.
...
- **`summary_ko` 각 불릿이 `• **키워드**: ` 형식으로 시작하지 않음**
