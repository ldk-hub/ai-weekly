...
2026-07-27 런의 Rising 상위가 전부 60k~96k star 리포였던 원인이 여기다. 따라서
다음을 **추가로** 실행:
- `--sort updated` 로 각 키워드 1회씩
- `created:>{30일전}` + `stars:>=3` 저star 신상 쿼리
- 노이즈 필터(`stars < 3` AND 3개월 방치)는 그대로 유지

**"성장률로 재정렬해도 순위가 같다"는 옛 메모는 틀렸다** (2026-08-04 정정).
당시 후보 풀이 이미 거대 리포뿐이어서 성장률이 구별력을 낼 대상이 없었을 뿐이다.
2026-08-03 풀(59건, 소형 리포 포함)로 재계산하니 상위 22 중 9건이 pending 에서
승격하고 성장률 2% 미만 거대 리포 13건이 탈락했다. 수집 편향과 공식 편향은 **둘
다** 고쳐야 하는 별개 문제다 (`trend-scoring` 의 성장률 절 참조).

### 커뮤니티에서만 발견된 리포도 후보로 올린다 (2026-08-04)
`community-scout` 가 찾았지만 GitHub 검색에 안 걸린 리포가 채점 자체에서 빠진다.
2026-08-03 런: `yc-software/qm` 은 `02_community_raw.json`·GeekNews 코퍼스에 있었지만
`01_github_raw.json` 에 없어 점수판에 오르지 못했다 (같은 주 weeklaude 는 이 리포를
Rising 1위로 발행 — HN 프론트 665pts). `ayghri/i-have-adhd` 도 동일 경로로 누락.

community-scout 가 먼저 돌았다면 `02b_repo_mentions.json` 의 리포 ID 를 전부
`gh api repos/{id}` 로 조회해 `01_github_raw.json` 에 합친다. 아직 안 돌았다면
trend-analyzer 가 병합 책임을 진다 — **어느 쪽이든 커뮤니티 발견분이 채점 풀에
들어간 것을 확인하고 보고**할 것.
...
- **발행 하한선 `stars >= 500`** — 수집 풀에서는 빼지 말고 **분석·발행 단계에서** 제외한다(원장 기준선과 코호트 중앙값을 지키기 위해. `trend-scoring` 의 '발행 하한선' 절 참조). 2026-08-24·25 런이 1★ `workflow-gate`·1★ `resume-audit`·2★ `claude-skills` 를 rising 으로 발행한 경로가 여기다 — 스크립트 경로(`collect.js`)에는 하한선이 있었고 이 에이전트 풀에는 없었다(실측: `_workspace/01_github_raw.json` 445건 중 3★ 미만 98건)
